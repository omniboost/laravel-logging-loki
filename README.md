...
| `debug` | Report what the package is doing (failures are reported regardless of this) | `false` |
...
Either way the job's `failed()` handler runs and the failure is reported through
the package's self-log. To handle these centrally, match on the exception class in
your application's exception handler or in a queue `JobFailed` listener.
...
the default channel - the usual setup, a `stack` channel in
`config/logging.php` listing both `stderr` and `omniboost:loki`, selected with
`LOG_CHANNEL` - a Loki failure written with `Log::error()` is buffered for Loki,
dispatches another push job, fails again, and reports that failure the same way.
Nothing recurses infinitely because the push is queued, but every outage
amplifies itself until the buffer or the queue gives out.
...
Its lines are prefixed with the level they were reported at, so
`grep '\[loki:'` finds all of them and `grep '\[loki:error\]'` finds only the
failures.
...
One that does is refused with a line in the error log, which is used instead.
The check follows a channel to the Loki driver through stacks (including the
stack the application's default channel points at), and recognises Laravel's
`monolog` and `custom` drivers configured with the package's
`LokiBufferedLogger` handler. A `custom` driver whose factory builds that handler
in code is invisible from the config, so do not rely on the check: it is a guard
rail, not a configuration option.
